...
To start the backend, `cd` into `react/backend` and run `npm install`, then `npm run start`.
To start the frontend, `cd` into `react/frontend` and run `npm install`, then `npm run dev`.
...
On this new page, attempt to fetch `localhost:5005/api/secret-message` from the backend.
